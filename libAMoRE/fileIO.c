...
static void writerex(regex crex)
{
	...
	for (i = 0; i <= crex->useda; i++) {
		fwrite(pi2a(crex->abbl[i]));
		new_line;
	}
	fwrite(text[REGNO + 4]);
	for (i = 0; i <= crex->useda; i++) {
		writestr(crex->abbr[i], crex->abbl[i]);
		new_line;
	}
	fwrite(text[REGNO + 5]);
...
static void writedcl(dstruct cdcl)
{
	...
	register darray dc;
	fwrite(text[DCLNO]);
	fwrite(pi2a(cdcl->dno));
	new_line;
	fwrite(text[DCLNO + 1]);
	fwrite(pi2a(cdcl->rno));
	new_line;
	fwrite(text[DCLNO + 2]);
	fwrite(pi2a(cdcl->lno));
	new_line;
...
/*	fwrite(text[DCLNO+3]);
	...
	for (i = 0; i < cdcl->dno; i++) {
		fwrite(text[DCLNO + 5]);
		fwrite(pi2a(dc[i]->org));
		new_line;
		fwrite(pi2a(dc[i]->rno));
		new_line;
		fwrite(pi2a(dc[i]->lno));
		new_line;
		fwrite(pi2a(dc[i]->hsize));
		new_line;
		fwrite(pi2a(dc[i]->rang));
		new_line;
		fwrite(pi2a(dc[i]->maxlen));
		new_line;
		fwrite(be2a(dc[i]->regular));
		fwrite(text[DCLNO + 6]);
		for (j = 0; j < dc[i]->lno;) {
			fwrite(pi2a(dc[i]->lrep[j++]));
			new_line;
		}
		fwrite(text[DCLNO + 7]);
		for (j = 0; j < dc[i]->rno;) {
			fwrite(pi2a(dc[i]->rrep[j++]));
			new_line;
		}
		fwrite(text[DCLNO + 8]);
		for (j = 0; j < dc[i]->hsize;) {
			fwrite(pi2a(dc[i]->hclass[j++]));
			new_line;
		}
	}
}				/* writedcl */
...
static void writemon(register monoid cmon)
{
	...
	new_line;
	fwrite(text[MONNO + 4]);
	fwrite(pi2a(cmon->gno));
	new_line;
	fwrite(text[MONNO + 8]);
	fwrite(pi2a(cmon->zero));
	new_line;
	fwrite(text[MONNO + 14]);

	/* oma */
	if(cmon->relationcomputed) {
		fwrite(pi2a(cmon->rs->rno));
		new_line;
		for (i = 0; i < cmon->rs->rno; i++) {
			fwrite(pi2a(cmon->rs->rside[i]));
			new_line;
			fwrite(pi2a(cmon->rs->lside[i]));
			new_line;
		}
	} else {
		fwrite(pi2a(0));
		new_line;	/* cmon->rs->rno == 0 indicates that 
...
static void writesf(register starfexp csf, posint mno)
{
	...
	fwrite(text[SFENO]);
	for (i = 0; i < mno;) {
		fwrite(pi2a(csf->ulength[i++]));
		new_line;
	}
	fwrite(text[SFENO + 1]);
	for (i = 0; i < mno;) {
		fwrite(pi2a(csf->vlength[i++]));
		new_line;
	}
	fwrite(text[SFENO + 2]);
	for (i = 0; i < mno;) {
		fwrite(pi2a(csf->w0length[i++]));
		new_line;
	}
	fwrite(text[SFENO + 3]);
	for (i = 0; i < mno;) {
		fwrite(pi2a(csf->w1length[i++]));
		new_line;
	}
	fwrite(text[SFENO + 4]);
	for (i = 0; i < mno; i++)
		for (j = 0; j < 2 * csf->ulength[i];) {
			fwrite(pi2a(csf->u[i][j++]));
			new_line;
		}
	fwrite(text[SFENO + 5]);
	for (i = 0; i < mno; i++)
		for (j = 0; j < 2 * csf->vlength[i];) {
			fwrite(pi2a(csf->v[i][j++]));
			new_line;
		}
	fwrite(text[SFENO + 6]);
	for (i = 0; i < mno; i++)
		for (j = 0; j < 3 * csf->w0length[i];) {
			fwrite(pi2a(csf->w0[i][j++]));
			new_line;
		}
	fwrite(text[SFENO + 7]);
	for (i = 0; i < mno; i++)
		for (j = 0; j < csf->w1length[i];) {
			fwrite(pi2a(csf->w1[i][j++]));
			new_line;
		}
	fwrite(text[SFENO + 8]);
	for (i = 0; i < mno;)
		fwrite(be2a(csf->computed[i++]));
	fwrite(text[SFENO + 9]);
}				/* writesf */
...
boolx writelan(language lanout, char *workdir)
{
	...
	fwrite(text[GLBNO]);
	fwrite(versiontext[CUR_VERSION]);
	fwrite(text[GLBNO + 1]);
	fwrite(lanout->name);
	new_line;
	fwrite(lanout->ref);
	...
	case NFAINP:
		break;
	case OREXINP:
		writerex(lanout->orex);
		break;
	case ODFAINP:
		writedfa(lanout->odfa);
		break;
	case ONFAINP:
		writenfa(lanout->onfa);
		break;
	case OENFAINP:
		writenfa(lanout->oenfa);
		break;
	default:
		errex(SWERR, "WRITELAN");
	}
	fwrite(text[GLBNO + 4]);
	fwrite(text[GLBNO + 5]);
	fwrite(be2a(lanout->brex));
	if(lanout->brex)
		writerex(lanout->lrex);
	fwrite(text[GLBNO + 6]);
	fwrite(be2a(lanout->bdfa));
	if(lanout->bdfa)
		writedfa(lanout->ldfa);
	fwrite(text[GLBNO + 7]);
	fwrite(be2a(lanout->bnfa));
	if(lanout->bnfa)
		writenfa(lanout->lnfa);
	fwrite(text[GLBNO + 8]);
	fwrite(be2a(lanout->bmon));
	if(lanout->bmon)
		writemon(lanout->lmon);
	fwrite(text[GLBNO + 9]);
	fwrite(be2a(lanout->stariscomp));
	if(lanout->stariscomp)
		writesf(lanout->sexp, lanout->lmon->mno);
...
static boole rdinsf(register starfexp csf, posint mno)
{
	...
	frdin();
	if(strcmp(line, text[SFENO]) != 0) {
		freesf(csf, FALSE, mno);
		return FALSE;
	}
	for (i = 0; i < mno;) {
		frdin();
		csf->ulength[i++] = a2pi(line);
	}
	/* "STARFREE EXPRESSION: LENGTH OF V\n" */
	frdin();
	if(strcmp(line, text[SFENO + 1]) != 0) {
		freesf(csf, FALSE, mno);
		return FALSE;
	}
	for (i = 0; i < mno;) {
		frdin();
		csf->vlength[i++] = a2pi(line);
	}
	/* "STARFREE EXPRESSION: LENGTH OF W0\n" */
	frdin();
	if(strcmp(line, text[SFENO + 2]) != 0) {
		freesf(csf, FALSE, mno);
		return FALSE;
	}
	for (i = 0; i < mno;) {
		frdin();
		csf->w0length[i++] = a2pi(line);
	}
	/* "STARFREE EXPRESSION: LENGTH OF W1\n" */
	frdin();
	if(strcmp(line, text[SFENO + 3]) != 0) {
		freesf(csf, FALSE, mno);
		return FALSE;
	}
	for (i = 0; i < mno;) {
		frdin();
		csf->w1length[i++] = a2pi(line);
	}
	...
	for (i = 0; i < mno; i++)
		for (j = 0; j < 3 * csf->w0length[i];) {
			frdin();
			csf->w0[i][j++] = a2pi(line);
		}
	/* "STARFREE EXPRESSION: W1\n" */
	frdin();
	if(strcmp(line, text[SFENO + 7]) != 0) {
		freesf(csf, TRUE, mno);
		return FALSE;
	}
	for (i = 0; i < mno; i++)
		for (j = 0; j < csf->w1length[i];) {
			frdin();
			csf->w1[i][j++] = a2pi(line);
		}
	/* "STARFREE EXPRESSION: COMPUTED\n" */
	frdin();
	if(strcmp(line, text[SFENO + 8]) != 0) {
		freesf(csf, TRUE, mno);
		return FALSE;
	}
	for (i = 0; i < mno;) {
		frdin();
		csf->computed[i++] = a2be(line);
	}
	/* "STARFREE EXPRESSION: END\n" */
	frdin();
	if(strcmp(line, text[SFENO + 9]) != 0) {
		freesf(csf, TRUE, mno);
		return FALSE;
	}
...
static boole rdinrex(register regex crex)
{
	...
	crex->rexl = a2pi(line);
	frdin();
	crex->erexl = a2pi(line);
	/* "REG.EXP.: REX KNOWN, GENERALIZED REX\n" */
	frdin();
	if(strcmp(line, text[REGNO + 2]))
		return FALSE;
	/* ??? skipping "rex known?" */
	frdin();
	frdin();
	...
	/* "REG.EXP.: LENGTH OF ABBREVIATIONS\n" */
	frdin();
	if(strcmp(line, text[REGNO + 3]))
		return FALSE;
	/* fill abbl(array that holds lengths of abbrs */
	for (i = 0; i <= crex->useda; i++) {
		frdin();
		crex->abbl[i] = a2pi(line);
	}
	/* "REG.EXP.: ABBREVIATIONS\n" */
	frdin();
	if(strcmp(line, text[REGNO + 4]))
		return FALSE;
...
static boole rdinnfa(nfa cnfa)
{
	...
	if(strcmp(line, text[NFANO + 4]) != 0)
		return FALSE;
	frdin();
	cnfa->is_eps = a2be(line);
	if(version < 1) {
		frdin();	/* reada dummy */
	}
	frdin();
	cnfa->minimal = a2be(line);
	/* "NFA: FINAL STATES\n" */
	frdin();
...
static boole rdindcl(dstruct * c)
{
	...
	for (i = 0; i < (*c)->dno; i++) {
		...
		dc[i]->lno = a2pi(line);
		frdin();
		dc[i]->hsize = a2pi(line);
		frdin();
		dc[i]->rang = a2pi(line);
		frdin();
		dc[i]->maxlen = a2pi(line);
		frdin();
		dc[i]->regular = a2be(line);
		...
		/* "D-CLASS THE CLASSES: L-REP\n" */
		frdin();
		if(strcmp(line, text[DCLNO + 6]) != 0) {
			freedcl((*c), i + 1);
			return FALSE;
		}
		for (j = 0; j < dc[i]->lno;) {
			frdin();
			dc[i]->lrep[j++] = a2pi(line);
		}
		/* "D-CLASS THE CLASSES: R-REP\n" */
		frdin();
		if(strcmp(line, text[DCLNO + 7]) != 0) {
			freedcl((*c), i + 1);
			return FALSE;
		}
		for (j = 0; j < dc[i]->rno;) {
			frdin();
			dc[i]->rrep[j++] = a2pi(line);
		}

		/* "D-CLASS THE CLASSES: H-CLASS\n" */
		frdin();
		if(strcmp(line, text[DCLNO + 8]) != 0) {
			freedcl((*c), i + 1);
			return FALSE;
		}
		for (j = 0; j < dc[i]->hsize;) {
			frdin();
			dc[i]->hclass[j++] = a2pi(line);
		}
	}
	return TRUE;
}				/* rdindcl */
...
static boole rdinmon(register monoid cmon)
{
	...
	/* "MONOID: #GENERATORS\n" */
	frdin();
	if(strcmp(line, text[MONNO + 4]) != 0)
		return FALSE;
	frdin();
	cmon->gno = a2pi(line);
	/* "MONOID: ZERO of THE MONOID\n" */
	frdin();
	if(strcmp(line, text[MONNO + 8]) != 0)
		return FALSE;
	frdin();
	cmon->zero = a2pi(line);
	/* "MONOID: #RELATIONS, (RIGHT,LEFT)-SIDE\n" */
	frdin();
	if(strcmp(line, text[MONNO + 14]) != 0)
		return FALSE;
	frdin();
	rno = a2pi(line);	/* if 0 then defining relations have
				   not been computed */
	...
	if(rno != 0) {
		...
		for (i = 0; i < rno; i++) {
			frdin();
			cmon->rs->rside[i] = a2pi(line);
			frdin();
			cmon->rs->lside[i] = a2pi(line);
		}
	} else {
	...
	/* "MONOID: LIST OF GENERATORS\n" */
	frdin();
	if(strcmp(line, text[MONNO + 5]) != 0)
		return FALSE;
	for (i = 0; i <= cmon->gno;) {
		frdin();
		cmon->generator[i++] = a2pi(line);
	}
	/* "MONOID: LETTER TO GENERATOR WITH SAME TRANSFORMATION\n" */
	frdin();
	if(strcmp(line, text[MONNO + 6]) != 0) {
		freemon(cmon);
		return FALSE;
	}
	for (i = 0; i <= cmon->alphabet_size;) {
	...
	for (i = 0; i < cmon->mno; i++) {
		for (j = 1; j <= cmon->gno;) {
			frdin();
			cmon->gensucc[i][j++] = a2pi(line);
		}
		frdin();
	}
	/* "MONOID: BOOLEAN VALUES (DCLASSISCOMPUTED, MEQUALS)\n" */
	frdin();
	if(strcmp(line, text[MONNO + 11]) != 0) {
		freemon(cmon);
		return FALSE;
	}
	frdin();
	cmon->dclassiscomputed = a2be(line);
	frdin();
	cmon->mequals = a2be(line);
	/* "MONOID: END\n" */
	frdin();
	if(strcmp(line, text[MONNO + 12]) != 0) {
		freemon(cmon);
		return FALSE;
	}
	...
	if(cmon->dclassiscomputed) {
		/* "MONOID: RANG\n" */
		frdin();
		if(strcmp(line, text[MONNO + 13]) != 0) {
			freemon(cmon);
			return FALSE;
		}
		if(version < 1) {
...
language rdinlan(char *fname, char *workdir)
{
	...
	for (i = 0; i < NLEN; i++)
		lanin->name[i] = fname[i];	/* copy name */
	...
	/*  "ALPHABET \n" */
	frdin();
	if(strcmp(line, text[GLBNO + 2]) != 0)
		return NULL;
	/* read pos. int */
	frdin();
	lanin->sizeal = a2pi(line);
	/*   "INPUT\n" */
	frdin();
	if(strcmp(line, text[GLBNO + 3]) != 0)
		return NULL;
	/* where is the user input ? read integer */
	frdin();
	lanin->inputis = a2i(line);
	...
	case DFAINP:
	case NFAINP:
		break;		/* if user entered fa, do nothing ???? */

	case OREXINP:
		lanin->orex = newrex();	/* original rex */
	...
	case ODFAINP:
		lanin->odfa = newdfa();	/* original dfa */
	...
	case ONFAINP:
		lanin->onfa = newnfa();	/* original nfa */
	...
	case OENFAINP:
		lanin->oenfa = newnfa();	/* original eps-nfa */
	...
	/* "BOOLE FLAGS: (BREX,BDFA,BNFA,BENFA,BMON) \n"   ((completely useless)) */
	frdin();
	if(strcmp(line, text[GLBNO + 4]) != 0)
		return NULL;
	/* "REGULAR EXPRESSION \n" */
	frdin();
	if(strcmp(line, text[GLBNO + 5]) != 0)
		return NULL;

	/* read 'boole' (in a2be ... "T" = True, else False) */
	frdin();
	lanin->brex = a2be(line);
	...
	/* "DETERMINISTIC FINITE AUTOMATON \n" */
	frdin();
	if(strcmp(line, text[GLBNO + 6]) != 0)
		return NULL;
	/* read 'boole' */
	frdin();
	lanin->bdfa = a2be(line);
	...
	/* "NONDETERMINISTIC FINITE AUTOMATON \n" */
	frdin();
	if(strcmp(line, text[GLBNO + 7]) != 0)
		return NULL;
	/* read 'boole' */
	frdin();
	lanin->bnfa = a2be(line);
	...
	/* "MONOID \n" */
	frdin();
	if(strcmp(line, text[GLBNO + 8]) != 0)
		return NULL;
	/* read boole */
	frdin();
	lanin->bmon = a2be(line);
	...
	/* "STARFREE EXPRESSION \n" */
	frdin();
	if(strcmp(line, text[GLBNO + 9]) != 0)
		return NULL;
	/* read 'boole' */
	frdin();
	lanin->stariscomp = a2be(line);
	...
	/* "EMPTY,FULL,FOLU,SF,LOC.TESTABLE,DEFINITE,REV.D.,GEN.D.,DOTDEPTH,NILPOTENT\n" */
	frdin();
	if(strcmp(line, text[GLBNO + 10]) != 0)
		return NULL;
	/* read 'boolx' types (True, False, Unknown) and one pos. int */
	frdin();
	lanin->empty = a2bx(line);
	frdin();
	lanin->full = a2bx(line);
	frdin();
	lanin->folu = a2bx(line);
	frdin();
	lanin->starfree = a2bx(line);
	frdin();
	lanin->localtest = a2bx(line);
	frdin();
	lanin->definite = a2bx(line);
	frdin();
	lanin->revdefinite = a2bx(line);
	frdin();
	lanin->gendefinite = a2bx(line);
	frdin();
	lanin->dotdepth1 = a2bx(line);
	frdin();
	lanin->piecetest = a2bx(line);
	frdin();
	lanin->nilpotent = a2bx(line);
	frdin();
	lanin->localdegree = a2pi(line);
